// ===============================
// admin.js - Código completo unificado
// ===============================

// ===============================
// Import / Declaraciones iniciales
// ===============================
let registeredTeams = JSON.parse(localStorage.getItem("registeredTeams")) || []
const XLSX = window.XLSX || null // Si cargas sheetjs en el HTML, estará en window.XLSX
...
function syncData(type, data) {
  try {
    ...
    console.log("[v0] Syncing data:", type)
  } catch (err) {
...
syncChannel.onmessage = (event) => {
  ...
  switch (type) {
    ...
    case "refresh":
      ...
      break
    default:
      console.log("[v0] Unknown sync type:", type)
  }
}
...
function initForms() {
  ...
  if (heroStatsForm) {
    heroStatsForm.addEventListener("submit", (e) => {
      e.preventDefault()
      heroStats = {
...
// ===============================
// Registered Teams (sección nueva solicitada)
// ===============================
...
function deleteRegisteredTeam(teamId) {
  ...
  renderRegisteredTeams()
  syncData("registeredTeams", registeredTeams)
  syncData("refresh", null)
}
...
function saveTeamData(team, teamIndex) {
  if (teamIndex >= 0) {
...
function loadPlayerStats() {
  ...
  // Poblar con equipos registrados
  statsTeam.innerHTML =
  ...
  // Event listener para cargar jugadores al seleccionar equipo
...
function updatePlayerStatsFromRegistered() {
...
function saveMatch() {
  ...
  if (phase === "groups" && winner) {
    ...
    const loserTeam = registeredTeams.find((t) => t.id === (winner === team1 ? team2 : team1))
  ...
  syncData("matches", matches)
  syncData("tournamentState", tournamentState)
  loadMatches()
  const matchForm = document.getElementById("matchForm")
...
function updateTournamentState(phase, winnerId) {
...
function saveMapBan() {
  ...
  mapBans.push(ban)
  localStorage.setItem("mapBans", JSON.stringify(mapBans))
  syncData("mapBans", mapBans)
  loadMapBans()
...
// ===============================
// Update selects utility (original)
// ===============================
...
// ===============================
// Edit Teams Section Initialization
// ===============================
